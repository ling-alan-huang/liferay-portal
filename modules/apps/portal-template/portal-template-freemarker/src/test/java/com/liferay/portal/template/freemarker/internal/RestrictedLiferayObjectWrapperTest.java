...
public class RestrictedLiferayObjectWrapperTest extends BaseObjectWrapperTest {
	...
	public void testRestrictedMethodNamesIncorrectSyntax() throws Exception {
		try (CaptureHandler captureHandler =
			...
			Assert.assertEquals(
				"\"" + methodName +
					"\" does not match format \"className#methodName\"",
				logRecord.getMessage());
		}
	}
	...
	private void _testRestrictedMethodNames(
		...
		try {
			...
			sb.append("Denied access to method or field ");
			sb.append(key);
			sb.append(" of ");
			sb.append(TestLiferayMethodObject.class.toString());

			Assert.assertEquals(sb.toString(), tme.getMessage());
		}
	}
