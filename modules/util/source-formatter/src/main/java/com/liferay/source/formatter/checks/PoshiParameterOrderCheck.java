	...
	private String _sortPoshiParameter(String content) {
		...
		while (matcher1.find()) {
			...
			while (true) {
				...
				if (x == -1) {
					parameter = StringUtil.split(parameters, " = ");

					parametersMap.put(parameter[0].trim(), parameter[1]);
...
//				String parameter = parameters.substring(0, x);
				parameter = StringUtil.split(parameters.substring(0, x), " = ");
			...
			content = StringUtil.replaceFirst(
				content, matcher1.group(2), sb.toString());
